...
const web3 = new Web3();
web3.setProvider(new web3.providers.HttpProvider('http://104.215.21.5:22000'));
...
web3.eth.defaultAccount="0x4F03916F01C51d676A3b83c637000EA16A6de51B";
...
async function runtime(){
    for(let i = 0;i < 100;i++){
        ...
        const response = await fetch('http://104.215.21.5:10001/send');
        end = performance.now();
...
async function runtime_pre(){
    for(let i = 0;i < 100;i++){
        let start,end;
        start = performance.now();
        const response = await fetch('http://104.215.21.5:10001/pre');
        end = performance.now();
        console.log((end - start) / 1000);
    }
}
...
const set = async () => {
    for(let i = 0;i < 100;i++){
        let start,end;
        start = performance.now();
        const response = await fetch('http://104.215.21.5:10001/set');
        end = performance.now();
        console.log((end - start) / 1000);
    }
};
...
const setDirectly = async () => {
    ...
    for(let i = 0;i < 300;i++){
        ...
        .then(receipt => {
            ...
            fs.appendFileSync('./time/9.txt', time + '\n', 'utf-8');
        });
    }
}
...
const setDirectlyFromPoisson = async () => {
    ...
    for(const index in arr){
        ...
        fs.appendFileSync('./time/9.txt', Date.now() - beginTime + '\n', 'utf-8');
