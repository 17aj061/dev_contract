...
const web3 = new Web3();
web3.setProvider(new web3.providers.HttpProvider('http://104.215.21.27:22000'));
...
web3.eth.defaultAccount="0x6d07E917714C3925f9E81f9D0589f570F01Cd200";
...
async function runtime(){
    for(let i = 0;i < 100;i++){
        ...
        const response = await fetch('http://104.215.21.27:10001/send');
        end = performance.now();
...
async function runtime_pre(){
    for(let i = 0;i < 100;i++){
        let start,end;
        start = performance.now();
        const response = await fetch('http://104.215.21.27:10001/pre');
        end = performance.now();
        console.log((end - start) / 1000);
    }
}
...
const set = async () => {
    for(let i = 0;i < 100;i++){
        let start,end;
        start = performance.now();
        const response = await fetch('http://104.215.21.27:10001/set');
        end = performance.now();
        console.log((end - start) / 1000);
    }
};
...
const setDirectly = async () => {
    ...
    for(let i = 0;i < 300;i++){
        ...
        .then(receipt => {
            ...
            fs.appendFileSync('./time/6.txt', time + '\n', 'utf-8');
        });
    }
}
...
const setDirectlyFromPoisson = async () => {
    ...
    for(const index in arr){
        ...
        fs.appendFileSync('./time/6.txt', Date.now() - beginTime + '\n', 'utf-8');
