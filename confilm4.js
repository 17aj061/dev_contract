...
web3.eth.defaultAccount="0x0037A9A4fdCb7B72019fdF098F9cdE2868628a06";
...
async function runtime(){
    for(let i = 0;i < 100;i++){
        ...
        const response = await fetch('http://104.215.20.64:10001/send');
        end = performance.now();
...
async function runtime_pre(){
    for(let i = 0;i < 100;i++){
        let start,end;
        start = performance.now();
        const response = await fetch('http://104.215.20.64:10001/pre');
        end = performance.now();
        console.log((end - start) / 1000);
    }
}
...
const set = async () => {
    for(let i = 0;i < 100;i++){
        let start,end;
        start = performance.now();
        const response = await fetch('http://104.215.20.64:10001/set');
        end = performance.now();
        console.log((end - start) / 1000);
    }
};
...
const setDirectly = async () => {
    ...
    for(let i = 0;i < 300;i++){
        ...
        .then(receipt => {
            ...
            fs.appendFileSync('./time/4.txt', time + '\n', 'utf-8');
        });
    }
}
