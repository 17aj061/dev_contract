...
contract Authentication{
    ...
    function set_tx(string memory _hash,address _address) public {
        transactions.push(Transaction(_hash , _address));
    }

    //認証部分
    function auth(bytes32 _hash,address _address) public view returns (bool){
        uint tx_count = 0;
        ...
        for(uint i = transactions.length - 1; i >= 0; i--){
            //アドレスが一致しているかの確認
            if(keccak256(abi.encodePacked(transactions[i].from)) == keccak256(abi.encodePacked(_address))){
